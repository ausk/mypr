...
Mat colorMatch(const Mat &src, Mat &match, const Color r,
  ...
  // Get the mask of the input image that fits the given HSV region.
  ...
  const std::map<int, std::vector<int>> HVals = {
      {BLUE, {100, 140}},
      {YELLOW, {15, 40}},
      {WHITE, {0, 30}},
      {UNKNOWN, {0, 0}},
  };
  int min_h = HVals.at(r)[0];
  int max_h = HVals.at(r)[2];
  float diff_h = float((max_h - min_h) / 2);
  float avg_h = float((max_h + min_h) / 2);
  ...
  for (i = 0; i < nRows; ++i) {
    ...
    for (j = 0; j < nCols; j += 3, p += 3) {
      int H = int(p[0]); // int(p[j]);     // 0-180
      int S = int(p[1]); // int(p[j + 1]); // 0-255
      int V = int(p[2]); // int(p[j + 2]); // 0-255
      ...
      if (H > min_h && H < max_h) {
        ...
        if (true == adaptive_minsv) {
          float Hdiff = H > avg_h ? (H - avg_h) : (avg_h - H);

          float Hdiff_p = float(Hdiff) / diff_h;
          min_sv =
              minref_sv * (1 + Hdiff_p) / 2.0; // minref_sv / 2 * (1 + Hdiff_p)
        } else {
...
bool bFindLeftRightBound(const Mat &bound_threshold, int &posLeft,
                         int &posRight) {

  int STEP = 2;
  int SPAN = int(bound_threshold.rows * 0.2f);
  float TH1 = 0.15f, TH2 = 0.10f;

  for (int i = 0; i < bound_threshold.cols - SPAN - 1; i += STEP) {
    int whiteCount = 0;
    ...
    if (whiteCount * 1.0 / (SPAN * bound_threshold.rows) > TH1) {
      posLeft = i;
      break;
    }
  }

  for (int i = bound_threshold.cols - 1; i > SPAN; i -= STEP) {
    int whiteCount = 0;
    for (int k = 0; k < bound_threshold.rows; k++) {
      for (int l = i; l > i - SPAN; l--) {
        if (bound_threshold.data[k * bound_threshold.step[0] + l] == 255) {
    ...
    if (whiteCount * 1.0 / (SPAN * bound_threshold.rows) > TH2) {
      posRight = i;
      break;
    }
  }
...
void clearLiuDing(Mat &mask, int &top, int &bottom) {
  ...
  std::vector<float> jump(mask.rows), white(mask.rows);
  ...
  for (int i = 0; i < mask.rows / 2; ++i) {
    if ((jump[i] < TH && white[i] * 1.0 / mask.cols > 0.15) || white[i] < 4) {
      top = i;
    }
  }
...
int getOtsuThreshold(const Mat &mat) {
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      unsigned char p = mat.at<unsigned char>(i, j);
      histogram[p]++;
    }
  }
  // normalize histogram
...
Rect getCenterRect(const Mat &in) {
  ...
  int step = in.step[0];
  // find the center valid rect
  // Top to down
  for (int i = 0, bFind = false; i < in.rows && !bFind; ++i) {
    for (int j = 0; j < in.cols; ++j) {
      if (in.data[i * step + j] > TH) {
        top = i;
        bFind = true;
        break;
      }
  ...
  // Down to top
  for (int i = in.rows - 1, bFind = false; i >= 0 && !bFind; --i) {
    for (int j = 0; j < in.cols; ++j) {
      if (in.data[i * step + j] > TH) {
        bottom = i;
        bFind = true;
  ...
  // Left to right
  for (int j = 0, bFind = false; j < in.cols && !bFind; ++j) {
  ...
  // Right to left
  for (int j = in.cols - 1, bFind = false; j >= 0 && !bFind; --j) {
    for (int i = 0; i < in.rows; ++i) {
      if (in.data[i * step + j] > TH) {
        right = j;
        bFind = true;
...
// 统计大于阈值的个数
int countBiggerValue(const Mat &mat, int iValue) {
  int iCount = 0;
  if (mat.rows > 1) {
...
Mat projectedHistogram(const Mat &img, int t) {
  ...
  for (int j = 0; j < sz; j++) {
    ...
    mhist.at<float>(j) = countBiggerValue(data, 20);
  }
...
bool verifyPlateSize(Rect mr) {
  ...
  float ratio = (float)mr.width / (float)mr.height;
  if (ratio < 1) {
    ratio = 1.0 / ratio;
  }

  if ((area < min || area > max) || (ratio < rmin || ratio > rmax)) {
    return false;
  } else {
    return true;
  }
...
bool verifyRotatedPlateSizes(RotatedRect mr) {
  ...
  if (area < min || area > max) {
    LOG(INFO) << "area < min || area > max: " << area << std::endl;
    return false;
  } else if (ratio < aspect_min || ratio > aspect_max) {
    LOG(INFO) << "ratio < aspect_min || ratio > aspect_max: " << ratio
              << std::endl;
    return false;
  } else if (angle < angle_min || angle > angle_max) {
    LOG(INFO) << "angle < angle_min || angle > angle_max: " << angle
              << std::endl;
    return false;
  } else if (width < width_min || width > width_max) {
    LOG(INFO) << "width < width_min || width > width_max: " << width
              << std::endl;
    return false;
  } else {
    return true;
...
void rotatedRectangle(InputOutputArray image, RotatedRect rrect,
                      const Scalar &color, int thickness, int lineType,
                      int shift) {
  Point2f rect_points[4];
  rrect.points(rect_points);
  for (int j = 0; j < 4; j++) {
    cv::line(image, rect_points[j], rect_points[(j + 1) % 4], color, thickness,
             lineType, shift);
  }
}
...
bool compareCharRect(const CCharacter &character1,
                     const CCharacter &character2) {
  ...
  double height_diff_ratio = abs(height_1 - height_2) / min(height_1, height_2);
  double width_diff_ratio = abs(width_1 - width_2) / min(width_1, width_2);

  if (height_diff_ratio > 0.25 || width_diff_ratio > 0.5) {
    return false;
  }
  ...
  double y_diff_ratio = abs(y_1 - y_2) / min(height_1, height_2);
  ...
  double x_diff_ratio = abs(x_1 - x_2) / min(height_1, height_2);
  ...
  double x_margin_diff_ratio =
      abs(x_margin_left - x_margin_right) / min(height_1, height_2);
...
void searchWeakSeed(const std::vector<CCharacter> &charVec,
                    std::vector<CCharacter> &mserCharacter, double thresh1,
  ...
  for (auto weakSeed : charVec) {
    ...
    Point weakCenter(weakRect.tl().x + weakRect.width / 2,
                     weakRect.tl().y + weakRect.height / 2);
  ...
  if (searchWeakSeedVec.size() != 0) {
    if (searchDirection == CharSearchDirection::LEFT) {
      std::sort(searchWeakSeedVec.begin(), searchWeakSeedVec.end(),
                [](const CCharacter &r1, const CCharacter &r2) {
                  return r1.getCharacterPos().tl().x >
                         r2.getCharacterPos().tl().x;
                });
    } else if (searchDirection == CharSearchDirection::RIGHT) {
      std::sort(searchWeakSeedVec.begin(), searchWeakSeedVec.end(),
                [](const CCharacter &r1, const CCharacter &r2) {
    ...
    float ratio =
        (float)abs(firstWeakCenter.x - boundaryPoint.x) / (float)maxrect.height;
    if (ratio > thresh2) {
      LOG(INFO) << "search seed ratio:" << ratio << std::endl;
      return;
    }
    ...
    for (size_t weakSeedIndex = 0; weakSeedIndex + 1 < searchWeakSeedVec.size();
         weakSeedIndex++) {
      ...
      if (x_margin_diff_ratio > thresh2) {
        ...
        LOG(INFO) << "search seed x_margin_diff_ratio:" << x_margin_diff_ratio
                  << std::endl;
...
void slideWindowSearch(const Mat &image,
                       std::vector<CCharacter> &slideCharacter,
                       const Vec4f &line, Point &fromPoint, const Vec2i &dist,
                       double ostu_level, float ratioWindow,
                       float threshIsCharacter, const Rect &maxrect,
                       Rect &plateResult, CharSearchDirection searchDirection,
                       bool isChinese, Mat &result) {
  float k = line[1] / line[0];
  float x_1 = line[2];
  float y_1 = line[3];
  ...
  for (int slideX = -slideLength; slideX < slideLength; slideX += slideStep) {
    ...
    Rect rect(Point2f(x_slide - chineseWidth / 2, y_slide - chineseHeight / 2),
              Size(chineseWidth, chineseHeight));

    if (rect.tl().x < 0 || rect.tl().y < 0 || rect.br().x >= image.cols ||
        rect.br().y >= image.rows) {
      continue;
    }
    ...
    Mat charInput = preRemapChar(binary_region, 20);
  ...
  for (auto character : charCandidateVec) {
    ...
    if (character.getCharacterScore() > threshIsCharacter &&
        character.getCharacterStr() != "1") {
      // cv::rectangle(result, rect, Scalar(255, 255, 255), 1);
      plateResult |= rect;
      slideCharacter.push_back(character);
      fromPoint = center;

      LOG(INFO) << "label:" << character.getCharacterStr();
      LOG(INFO) << "__score:" << character.getCharacterScore() << std::endl;
    }
  }
}

bool judegMDOratio2(const Mat &image, const Rect &rect,
                    std::vector<Point> &contour, Mat &result) {
...
Rect interRect(const Rect &a, const Rect &b) {
  ...
  c.width =
      (a.x + a.width < b.x + b.width ? a.x + a.width : b.x + b.width) - x1;
  c.height =
      (a.y + a.height < b.y + b.height ? a.y + a.height : b.y + b.height) - y1;
  c.x = x1;
  c.y = y1;
...
void removeRightOutliers(std::vector<CCharacter> &charGroup,
                         std::vector<CCharacter> &out_charGroup, double thresh1,
  ...
  float slope_last = 0;
  for (size_t charGroup_i = 0; charGroup_i + 1 < charGroup.size();
       charGroup_i++) {
    ...
    if (0) {
      cv::line(result, leftChar.getCenterPoint(), rightChar.getCenterPoint(),
               Scalar(0, 0, 255));
    }
  }
  ...
  for (size_t slopeVec_i = 0; slopeVec_i + 1 < slopeVec.size(); slopeVec_i++) {
    ...
    LOG(INFO) << "slope_diff:" << slope_diff << std::endl;
    if (slope_diff <= thresh1) {
      uniformity_count++;
    }
    LOG(INFO) << "slope_1:" << slope_1 << std::endl;
    LOG(INFO) << "slope_2:" << slope_2 << std::endl;
  ...
  LOG(INFO) << "uniformity_count:" << uniformity_count << std::endl;
  LOG(INFO) << "outlier_index:" << outlier_index << std::endl;

  for (int charGroup_i = 0; charGroup_i < (int)charGroup.size();
       charGroup_i++) {
...
// based on the assumptions: distance beween two nearby characters in plate are
// the same.
// add not found rect and combine two small and near rect.
void reFoundAndCombineRect(std::vector<CCharacter> &mserCharacter,
                           float min_thresh, float max_thresh, Vec2i dist,
                           Rect maxrect, Mat result) {
  ...
  for (; mserCharacter_i + 1 < mserCharacter.size(); mserCharacter_i++) {
    ...
    if (ratio > max_thresh) {
      ...
      Rect rect_union =
          leftChar.getCharacterPos() | rightChar.getCharacterPos();
      /*float x_add = (float)(leftCenter.x + rightCenter.x) / 2.f;
         float y_add = (float)(leftCenter.y + rightCenter.y) / 2.f;*/
...
void removeOutliers(std::vector<CCharacter> &charGroup, double thresh,
                    Mat result) {
  ...
  float y_1 = line[3];
  float step = 100;
  cv::line(result, Point2f(x_1 - step, y_1 - k * step),
           Point2f(x_1 + step, k * step + y_1), Scalar(0, 0, 255));
  ...
  for (auto character : charGroup) {
    ...
    float distance =
        (a * center.x + b * center.y + c) / std::sqrt(a * a + b * b);
...
void mserCharMatch(const Mat &src, std::vector<Mat> &match,
  ...
  mser->detectRegions(image, all_contours.at(0), all_boxes.at(0),
                      all_contours.at(1), all_boxes.at(1));
  ...
  for (int color_index = 0; color_index < 2; color_index++) {
    ...
    for (size_t index = 0; index < size; index++) {
      ...
      if (usePlateMser) {
        ...
        if (verifyRotatedPlateSizes(rrect)) {
          ...
          if (the_color == YELLOW) {
            out_plateRRect_yellow.push_back(rrect);
          }
        }
      }
      ...
      if (verifyCharSizes(rect)) {
        ...
        Mat charInput = preRemapChar(mserMat, char_size);
        Rect charRect = rect;

        Point center(charRect.tl().x + charRect.width / 2,
                     charRect.tl().y + charRect.height / 2);
        Mat tmpMat;
        double ostu_level = cv::threshold(image(charRect), tmpMat, 0, 255,
    ...
    for (auto charGroup : charGroupVec) {
      ...
      if (points.size() >= 2 && ratio_maxrect >= 0.3) {
        ...
        Point midCenter(midRect.tl().x + midRect.width / 2,
                        midRect.tl().y + midRect.height / 2);
        ...
        for (size_t mser_i = 0; mser_i + 1 < mserCharVec.size(); mser_i++) {
          ...
          Point centerCompare(
              charRectCompare.tl().x + charRectCompare.width / 2,
              charRectCompare.tl().y + charRectCompare.height / 2);

          int dist = charRectCompare.x - charRect.x;
          Vec2i distVec(charRectCompare.x - charRect.x,
                        charRectCompare.y - charRect.y);
        ...
        std::sort(
            distVecVec.begin(), distVecVec.end(),
            [](const Vec2i &r1, const Vec2i &r2) { return r1[0] < r2[0]; });
        ...
        // cv::line(result, Point2f(line[2] - step, line[3] - k*step),
        // Point2f(line[2] + step, k*step + line[3]), Scalar(255, 255, 255));
        cv::line(result, Point2f(midCenter.x - step, midCenter.y - k * step),
                 Point2f(midCenter.x + step, k * step + midCenter.y),
                 Scalar(255, 255, 255));
    ...
    for (auto plate : plateVec) {
      ...
      if (0 && showDebug) {
        ...
        ss << "resources/image/tmp/" << img_index << "_1_"
           << "searcgMserRect.jpg";
      ...
      // the same line as the strong seed. The judge condition contains the
      // distance
      // between strong seed and weak seed , and the rect simily of each other
      // to improve
      // the roubustnedd of the seed growing algorithm.
      if (mserCharacter.size() < char_max_count) {
        double thresh1 = 0.15;
        double thresh2 = 2.0;
        searchWeakSeed(searchCandidate, searchRightWeakSeed, thresh1, thresh2,
                       line, rightPoint, maxrect, plateResult, result,
                       CharSearchDirection::RIGHT);
        if (1 && showDebug) {
          std::cout << "searchRightWeakSeed:" << searchRightWeakSeed.size()
                    << std::endl;
        }
        ...
        searchWeakSeed(searchCandidate, searchLeftWeakSeed, thresh1, thresh2,
                       line, leftPoint, maxrect, plateResult, result,
                       CharSearchDirection::LEFT);
        if (1 && showDebug) {
          std::cout << "searchLeftWeakSeed:" << searchLeftWeakSeed.size()
                    << std::endl;
        }
      ...
      // and sometimes two strong seed are actually the two parts of one
      // character.
      // because we only consider the weak seed in the left and right direction
      // of strong seed.
      // now we examine all the strong seed and weak seed. not only to find the
      // seed in the middle,
      // but also to combine two seed which are parts of one character to one
      // seed.
      // only by this process, we could use the seed count as the condition to
      // judge if or not to use slide window.
      float min_thresh = 0.3f;
      float max_thresh = 2.5f;
      reFoundAndCombineRect(mserCharacter, min_thresh, max_thresh, dist,
                            maxrect, result);

      // if the characters count is less than max count
      // this means the mser rect in the lines are not enough.
      // sometimes there are still some characters could not be captured by mser
      // algorithm,
      ...
      if (mserCharacter.size() < char_max_count) {
        ...
        if (1) {
          ...
          // Rect theRect =
          // adaptive_charrect_from_rect(leftChar.getCharacterPos(), image.cols,
          // image.rows);
          ...
          ostu_level = cv::threshold(region, binary_region, 0, 255,
                                     CV_THRESH_BINARY | CV_THRESH_OTSU);
          if (1 && showDebug) {
          ...
          Mat charInput = preRemapChar(binary_region, char_size);
          if (0 /*&& showDebug*/) {
          ...
          std::string label = "";
          float maxVal = -2.f;
          leftIsChinese = CharsIdentify::instance()->isCharacter(
              charInput, label, maxVal, true);
          // auto character =
          // CharsIdentify::instance()->identifyChinese(charInput, maxVal,
          // leftIsChinese);
        ...
        if (!leftIsChinese) {
          slideWindowSearch(image, slideLeftWindow, line, leftPoint, dist,
                            ostu_level, ratioWindow, threshIsCharacter, maxrect,
                            plateResult, CharSearchDirection::LEFT, true,
                            result);
          if (1 && showDebug) {
            std::cout << "slideLeftWindow:" << slideLeftWindow.size()
                      << std::endl;
          }
          for (auto window : slideLeftWindow) {
            cv::rectangle(result, window.getCharacterPos(), Scalar(0, 0, 255),
                          1);
      ...
      if (mserCharacter.size() < char_max_count) {
        ...
        float threshIsCharacter = 0.8f;
        // float threshIsCharacter = CParams::instance()->getParam3f();
        slideWindowSearch(image, slideRightWindow, line, rightPoint, dist,
                          plate.getOstuLevel(), ratioWindow, threshIsCharacter,
                          maxrect, plateResult, CharSearchDirection::RIGHT,
                          false, result);
        if (1 && showDebug) {
          std::cout << "slideRightWindow:" << slideRightWindow.size()
                    << std::endl;
        }
      ...
      RotatedRect platePos(
          Point2f((float)plateResult.x + plateResult.width / 2.f,
                  (float)plateResult.y + plateResult.height / 2.f),
          Size2f(plateResult.width * widthEnlargeRatio,
                 maxrect.height * heightEnlargeRatio),
          angle);
      ...
      if (verifyRotatedPlateSizes(platePos)) {
        ...
        if (the_color == BLUE) {
          out_plateVec_blue.push_back(plate);
        }
        if (the_color == YELLOW) {
          out_plateVec_yellow.push_back(plate);
        }
      }
    ...
    if (1) {
      std::stringstream ss(std::stringstream::in | std::stringstream::out);
      ss << "resources/image/tmp/plateDetect/plate_" << img_index << "_"
         << the_color << ".jpg";
      imwrite(ss.str(), result);
    }
  }
...
void spatial_ostu(InputArray _src, int grid_x, int grid_y, Color type) {
  ...
  for (int i = 0; i < grid_y; i++) {
    for (int j = 0; j < grid_x; j++) {
      ...
      if (type == BLUE) {
        cv::threshold(src_cell, src_cell, 0, 255,
...
Rect adaptive_charrect_from_rect(const Rect &rect, int maxwidth,
                                 int maxheight) {
  int expendWidth = 0;
  ...
  int brx = rect.br().x + expendWidth < maxwidth ? rect.br().x + expendWidth
                                                 : maxwidth;
  int bry = rect.br().y;
...
Mat adaptive_image_from_points(
    ...
    const Scalar &forgroundColor /* = ml_color_black */,
    bool gray /* = true */) {
  int expendHeight = 0;
  int expendWidth = 0;
  ...
  Mat image(rect.height + expendHeight * 2, rect.width + expendWidth * 2,
            gray ? CV_8UC1 : CV_8UC3, backgroundColor);

  for (int i = 0; i < (int)points.size(); ++i) {
    Point point = points[i];
    Point currentPt(point.x - rect.tl().x + expendWidth,
                    point.y - rect.tl().y + expendHeight);
