...
Mat CCharsSegment::preRemapChar(Mat in) {
  // Remap image
  int h = in.rows;
  int w = in.cols;
...
void CCharsSegment::judgeChinese(Mat in, Mat &out, Color plateType) {
  ...
  if (1) {
    ...
    auto character =
        CharsIdentify::instance()->identifyChinese(roiOstu, valOstu, isChinese);
  }
  if (1) {
    if (BLUE == plateType) {
      adaptiveThreshold(auxRoi, roiAdap, 255, ADAPTIVE_THRESH_MEAN_C,
                        THRESH_BINARY, 3, 0);
    } else if (YELLOW == plateType) {
      adaptiveThreshold(auxRoi, roiAdap, 255, ADAPTIVE_THRESH_MEAN_C,
                        THRESH_BINARY_INV, 3, 0);
    } else if (WHITE == plateType) {
      adaptiveThreshold(auxRoi, roiAdap, 255, ADAPTIVE_THRESH_MEAN_C,
                        THRESH_BINARY_INV, 3, 0);
    } else {
    ...
    roiAdap = preRemapChar(roiAdap);
    auto character =
        CharsIdentify::instance()->identifyChinese(roiAdap, valAdap, isChinese);
  }
...
bool slideChineseWindow(Mat &image, Rect mr, Mat &newRoi, Color plateType,
                        float slideLengthRatio, bool useAdapThreshold) {
  ...
  for (int slideX = -slideLength; slideX < slideLength; slideX += slideStep) {
    ...
    Mat roiOstu, roiAdap;
    if (1) {
      static std::map<int, int> threshold_types = {
          {BLUE, CV_THRESH_BINARY + CV_THRESH_OTSU},
          {YELLOW, CV_THRESH_BINARY_INV + CV_THRESH_OTSU},
          {WHITE, CV_THRESH_OTSU + CV_THRESH_BINARY_INV},
          {UNKNOWN, CV_THRESH_OTSU + CV_THRESH_BINARY}};
      threshold(auxRoi, roiOstu, 0, 255, threshold_types[plateType]);

      roiOstu = preRemapChar(roiOstu, kChineseSize);
    ...
    if (useAdapThreshold) {
      static std::map<int, int> threshold_types = {{BLUE, THRESH_BINARY},
                                                   {YELLOW, THRESH_BINARY_INV},
                                                   {WHITE, THRESH_BINARY_INV},
                                                   {UNKNOWN, THRESH_BINARY}};

      adaptiveThreshold(auxRoi, roiAdap, 255, ADAPTIVE_THRESH_MEAN_C,
                        threshold_types[plateType], 3, 0);

      roiAdap = preRemapChar(roiAdap, kChineseSize);
  ...
  for (size_t i = 0; i < newSortedRect.size(); i++) {
    ...
    if (i == 0) {
      if (useSlideWindow) {
        ...
        if (!slideChineseWindow(input_grey, mr, newRoi, plateType,
                                slideLengthRatio, useAdapThreshold)) {
      ...
      threshold(auxRoi, newRoi, 0, 255, threshold_types[plateType]);

      newRoi = preRemapChar(newRoi);
    }
