...
bool changeDirectory(const std::string &folder) {
  bool ret = cd(folder.c_str()) == 0;
  if (ret) {

    LOG(ERROR) << "\nCannot change directory to :\n\t" << folder<<std::endl
               << "Current Working Directory:\n\t" << getCurrWorkingDirectory()
               << std::endl;
  }
  return ret;
};
...
std::size_t get_last_slash(const std::string &path) {
#ifdef OS_WINDOWS
...
std::string getFileName(const std::string &path, bool postfix) {
  if (path.empty()) {
    return "";
  }
...
bool imwrite(const std::string &file, const cv::Mat &image) {
  auto folder = file.substr(0, get_last_slash(file));
  mkdir(folder);
