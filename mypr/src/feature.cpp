...
void getHOGFeatures(const Mat &image, Mat &features) {
  // HOG descripter
  HOGDescriptor *hog =
      new HOGDescriptor(cvSize(128, 64), cvSize(16, 16), cvSize(8, 8),
                        cvSize(8, 8), 3); // these parameters work well
...
Mat charFeatures(const Mat &in, int sizeData) {
  const int VERTICAL = 0;
  const int HORIZONTAL = 1;
  ...
  int idx = 0;
  for (int i = 0; i < vhist.cols; ++i, ++idx) {
    out.at<float>(idx) = vhist.at<float>(i);
  }
  for (int i = 0; i < hhist.cols; ++i, ++idx) {
    out.at<float>(idx) = hhist.at<float>(i);
  }
  for (int x = 0; x < lowData.cols; x++) {
    for (int y = 0; y < lowData.rows; y++) {
      out.at<float>(idx) += (float)lowData.at<unsigned char>(x, y);
      ++idx;
    }
  }
  return out;
...
void getLBPplusHistFeatures(const Mat &image, Mat &features) {
  ...
  Mat lbpimage;
  lbpimage = libfacerec::olbp(grayImage.clone());
  ...
  /*Mat img_threshold2;
     ...
     Mat histomFeatures;
     hconcat(blueHistomFeatures.reshape(1, 1), greenHistomFeatures.reshape(1,
     1), histomFeatures);*/
