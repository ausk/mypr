...
CPlateRecognise::CPlateRecognise() { m_showResult = false; }

void groups_draw(Mat &src, std::vector<Rect> &groups) {
  for (int i = (int)groups.size() - 1; i >= 0; i--) {
    if (src.type() == CV_8UC3) {
      rectangle(src, groups.at(i).tl(), groups.at(i).br(), Scalar(0, 255, 255),
                3, 8);
    } else {
...
void er_show(std::vector<Mat> &channels,
  ...
  for (int c = 0; c < (int)channels.size(); c++) {
    ...
    for (int r = 0; r < (int)regions[c].size(); r++) {
      ...
      if (er.parent != NULL) { // deprecate the root region
        ...
        floodFill(channels[c], dst, Point(er.pixel % channels[c].cols,
                                          er.pixel / channels[c].cols),
...
int CPlateRecognise::plateRecognizeAsTextNM(Mat src,
  ...
  Ptr<text::ERFilter> er_filter1 =
      text::createERFilterNM1(text::loadClassifierNM1(pathMMmodel_1), 16,
                              0.00015f, 0.13f, 0.2f, true, 0.1f);
  Ptr<text::ERFilter> er_filter2 =
      text::createERFilterNM2(text::loadClassifierNM2(pathMMmodel_2), 0.5);
  ...
  // done in parallel)
  std::cout << "Extracting Class Specific Extremal Regions from "
            << (int)channels.size() << " channels ..." << std::endl;
  ...
  // text::ERGROUPING_ORIENTATION_HORIZ);
  // erGrouping(src, channels, regions, region_groups, groups_boxes,
...
int CPlateRecognise::plateRecognizeAsText(Mat src,
                                          std::vector<CPlate> &licenseVec) {
  // TODO
  return 0;
}

int CPlateRecognise::plateRecognize(Mat src, std::vector<CPlate> &plateVecOut,
                                    int img_index, Mat &output,
                                    const vector<CPlate> &plateVecGT) {
...
void CPlateRecognise::LoadSVM(std::string path) {
  CPlateJudge::instance()->LoadModel(path);
}

void CPlateRecognise::LoadANN(std::string path) {
  CharsIdentify::instance()->LoadModel(path);
}

void CPlateRecognise::LoadChineseANN(std::string path) {
  CharsIdentify::instance()->LoadChineseModel(path);
}

int CPlateRecognise::plateRecognize(Mat src,
