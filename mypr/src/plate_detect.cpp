...
std::shared_ptr<CPlateLocate> CPlateDetect::m_plateLocate =
    std::make_shared<CPlateLocate>(CPlateLocate());
...
#pragma omp section
  ...
  CPlateJudge::instance()->plateJudgeUsingNMS(all_result_Plates, resultVec,
                                              m_maxPlates);
...
int CPlateDetect::plateDetect(const Mat &  src, std::vector<CPlate> &resultVec,
                              int img_index) {
...
Mat CPlateDetect::showResult(const Mat &result, int img_index) {
  namedWindow("PlateRecognise", CV_WINDOW_AUTOSIZE);
  ...
  if (1) {
    imshow("PlateRecognise", img_window);
    waitKey(0);
    destroyWindow("PlateRecognise");
  }
