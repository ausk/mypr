...
bool CPlateLocate::verifySizes(RotatedRect mr) {
  ...
  float ratio = (float)mr.size.width / (float)mr.size.height;
  if (ratio < 1) {
    ratio = 1.0 / ratio;
  }

  return (area >= min && area <= max) && (ratio >= rmin && ratio <= rmax);
}

//! mser search method
int CPlateLocate::mserSearch(const Mat &src, vector<Mat> &out,
                             vector<vector<CPlate>> &out_plateVec,
                             bool usePlateMser,
  ...
  mserCharMatch(src, match_grey, plateVec_blue, plateVec_yellow, usePlateMser,
                plateRRect_blue, plateRRect_yellow, img_index, showDebug);
...
int CPlateLocate::sobelSecSearchPart(Mat &bound, Point2f refpoint,
  ...
  if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
    ...
    Util::imwrite("resources/image/tmp/repaireimg1.jpg", bound_threshold);
    ...
    Util::imwrite("resources/image/tmp/repaireimg2.jpg", bound_threshold);
  }
...
int CPlateLocate::sobelSecSearch(Mat &bound, Point2f refpoint,
  ...
  Util::imwrite("resources/image/tmp/sobelSecSearch.jpg", bound_threshold);
...
void deleteNotArea(Mat &inmat, Color color = UNKNOWN) {
  ...
  if (BLUE == plateType) {
    ...
    int threadHoldV = getOtsuThreshold(tmp);

    threshold(input_grey, img_threshold, threadHoldV, 255, CV_THRESH_BINARY);
    Util::imwrite("resources/image/tmp/inputgray2.jpg", img_threshold);
  ...
  if (bFindLeftRightBound(img_threshold, posLeft, posRight)) {
...
bool CPlateLocate::isdeflection(const Mat &in, const double angle,
  ...
  int nRows = in.rows, nCols = in.cols;
...
#pragma omp section
    {
      ...
      deskew(src_clone, src_b_yellow, rects_color_yellow, plates_yellow, true,
             YELLOW);
    }
  }

  candPlates.insert(candPlates.end(), plates_blue.begin(), plates_blue.end());
  candPlates.insert(candPlates.end(), plates_yellow.begin(),
                    plates_yellow.end());
...
//! MSER plate locate
int CPlateLocate::plateMserLocate(Mat src, vector<CPlate> &candPlates,
                                  int img_index) {
  ...
  for (size_t i = 0; i < channelImages.size(); ++i) {
    ...
    Mat image =
        scaleImage(channelImage, Size(scale_size, scale_size), scale_ratio);

    // vector<RotatedRect> rects;
    mserSearch(image, src_b_vec, platesVec, usePlateMser, plateRRectsVec,
               img_index, false);
    ...
    for (size_t j = 0; j < flags.size(); j++) {
      ...
      for (auto dplate : deskewPlate) {
        ...
        for (auto splate : mserPlate) {
          ...
          float iou = 0.f;
          bool isSimilar =
              computeIOU(drect, srect, src.cols, src.rows, 0.95f, iou);
          if (isSimilar) {
  ...
  //    if (color == BLUE)
  //      deskew(src, resize_src_b, plateRRect_B, deskewMserPlate, false,
  //      color);
  //    if (color == YELLOW)
  //      deskew(src, resize_src_b, plateRRect_Y, deskewMserPlate, false,
  //      color);
...
int CPlateLocate::sobelOperT(const Mat &in, Mat &out, int blurSize, int morphW,
  ...
  Util::imwrite("resources/image/tmp/grayblure.jpg", mat_gray);
  ...
  Util::imwrite("resources/image/tmp/graygrad.jpg", grad);

  Mat mat_threshold;
  double otsu_thresh_val =
      threshold(grad, mat_threshold, 0, 255, CV_THRESH_OTSU + CV_THRESH_BINARY);

  Util::imwrite("resources/image/tmp/grayBINARY.jpg", mat_threshold);

  Mat element = getStructuringElement(MORPH_RECT, Size(morphW, morphH));
  morphologyEx(mat_threshold, mat_threshold, MORPH_CLOSE, element);

  Util::imwrite("resources/image/tmp/phologyEx.jpg", mat_threshold);
...
#pragma omp critical
    {
      rects_sobel_all.insert(rects_sobel_all.end(), rects_sobel.begin(),
                             rects_sobel.end());
    }
  }
