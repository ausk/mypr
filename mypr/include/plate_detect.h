...
public:
  ...
  /** @brief Plate detect in an image.
     ...
     The function detects plate in an image. It can use sobel, color, and
     character method or the combinations of them.
  ...
  /** @brief Plate detect in an image.
     ...
     @param index
   */
  int plateDetect(const Mat & src, std::vector<CPlate> &resultVec, int img_index = 0);
...
private:
  ...
  static std::shared_ptr<CPlateLocate> m_plateLocate;

  int m_type;

  static std::string m_svmxml;

  // show the detect result image
  bool m_showDetect;
};
}
#endif // PLATE_DETECT_HPP
