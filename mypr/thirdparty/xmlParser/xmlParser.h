 ...
 * Commercialized by <a
 *href="http://www.Business-Insight.com">Business-Insight</a><br/>
 * See the file <a href="../../AFPL-license.txt">AFPL-license.txt</a> about the
 *licensing terms
 *
 ...
 * When I had to debug a software that was using the XMLParser Library, it was
 *usually
 * a nightmare because the library was sooOOOoooo slow in debug mode (because of
 *the
 * slow memory allocations in Debug mode). To solve this
 * problem, during all the debugging session, I am now using a very fast DLL
 *version of the
 * XMLParser Library (the DLL is compiled in release mode). Using the DLL
 *version of
 * the XMLParser Library allows me to have lightening XML parsing speed even in
 *debug!
 * Other than that, the DLL version is useless: In the release version of my
 *tool,
 * I always use the normal, ".cpp"-based, XMLParser Library (I simply include
 *the
...
public:
  ...
  XMLNode getChildNodeWithAttribute(XMLCSTR tagName, XMLCSTR attributeName,
                                    ...
                                    int *i = NULL) const; ///< return child node
                                                          ///with specific
                                                          ///name/attribute
                                                          ///(return an empty
                                                          ///node if failing)
...
XMLDLLENTRY XMLCHAR xmltoc(XMLCSTR xmlString,
                           const XMLCHAR defautValue = _CXML('\0'));
/** @} */
